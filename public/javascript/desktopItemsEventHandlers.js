...
import { readGuestData, writeGuestData } from './guestModeClient.js';
...
function handleMoveItem(e) {
	if (e.target.classList.contains('desktop-item')) {
		...
		const handleMouseUp = async (e) => {
			...
			if (document.body && document.body.classList.contains('guest-mode-active')) {
				const guestData = readGuestData() || {};
				if (guestData.user && guestData.user.desktopItems && guestData.user.desktopItems[targetId]) {
					guestData.user.desktopItems[targetId].xPosition = movementX;
					guestData.user.desktopItems[targetId].yPosition = movementY;
					if (Object.prototype.hasOwnProperty.call(guestData.user.desktopItems[targetId], 'rotation')) {
						const currentRotation = windowElement.style.rotate
							? Number(windowElement.style.rotate.replace('deg', ''))
							: guestData.user.desktopItems[targetId].rotation;
						guestData.user.desktopItems[targetId].rotation = Number.isNaN(currentRotation)
							? guestData.user.desktopItems[targetId].rotation
							: currentRotation;
					}
					writeGuestData(guestData);
				}
			}
		};
...
function handleDecimalRounding(
	decimalInvolved,
	cfvCdvNumStringLength,
	cfvCdvNumString,
	decimalStringLength,
	cfvCdvNum
) {
...
function doSpecVal() {
	if (
		calculatorDisplay.value === '5318008' ||
...
function handleCalculatorKeyboardEntry(e) {
	if (
		...
		// nodeName 'BUTTON' blocked only for e.key==='Enter' block below
		if (String(calculatorDisplay.value).replace('.', '').length >= 8) {
			calculatorDisplayAtMax = true;
		} else {
			calculatorDisplayAtMax = false;
